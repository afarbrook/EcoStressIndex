...
document.addEventListener("DOMContentLoaded", () => {
  map = L.map("map").setView([32.2226, -110.9747], 11);
...
async function searchCity(query) {
  ...
  try {
    ...
    if (data.city_center) {
      map.setView([data.city_center.lat, data.city_center.lon], 12);
    }
...
function renderNeighborhoods(neighborhoods) {
  ...
  if (neighborhoodLayers.length > 0) {
    try {
      const group = L.featureGroup(neighborhoodLayers);
